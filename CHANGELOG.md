...
构建 0.4.8 build 2 发布。包含重大内存优化、UI 性能重构及 TUN 稳定性增强。

### Fixed
- **极致轻量化与内存修复**:
  - 修复了连接列表中的长效内存泄露（移除无限增长的 `seenConnIDs` 集合）。
  - 增加流量历史记录自动清理逻辑（保留最近 60 天数据）。
  - 新增核心内存守护（Core Memory Guard）：当内核内存占用超过 512MB 时自动刷新 DNS/Fake-IP 缓存。
- **TUN 稳定性与自动化恢复**:
  - **网卡变更自动重绑**: 实时监控默认路由网卡变更（如 Wi-Fi 切换到以太网），自动更新 `interface-name` 并重置 TUN，防止流量黑洞。
  - 增强网络恢复后的主动同步机制，确保内核与 App 状态一致。
  - 修复了关闭代理或网络断开时 DNS 残留导致的系统断网问题。
- **UI 性能与卡死修复**:
  - **全系统异步化重构**: 将所有阻塞性系统工具调用（`route`, `networksetup`, `netstat` 等）改为 `async` 异步模式，消除 UI 微卡顿。
  - 修复了切换内核、下载内核或解压时导致的 UI 假死问题（耗时操作完全脱离主线程）。
  - 优化了流量数据刷新频率，降低了高负载下的 CPU 抖动。
